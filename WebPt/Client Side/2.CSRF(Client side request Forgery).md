...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      CSRF MITIGATION (TECHNICAL)                            │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   1. CSRF Tokens (Synchronizer Token Pattern)                               │
│      │                                                                      │
│      ├── Generate:                                                          │
│      │   Server creates cryptographically random token using secure         │
│      │   functions like bin2hex(random_bytes(32)) in PHP or                 │
│      │   crypto.randomBytes(32) in Node.js. Token must be unpredictable.    │
│      │                                                                      │
│      ├── Store:                                                             │
│      │   Token saved in server-side session storage, linked to user's       │
│      │   session ID. This creates a server-side reference for validation.   │
│      │                                                                      │
│      ├── Embed:                                                             │
│      │   Token placed in HTML forms as hidden input field. Browser          │
│      │   includes this token when form is submitted. Attacker's page        │
│      │   cannot access this token due to Same-Origin Policy.                │
│      │                                                                      │
│      ├── Validate:                                                          │
│      │   On form submission, server compares submitted token with           │
│      │   stored session token. Uses timing-safe comparison function         │
│      │   like hash_equals() to prevent timing attacks.                      │
│      │                                                                      │
│      └── Example:                                                           │
...
│      │                                                                      │
│      ├── Strict:                                                            │
│      │   Cookie is never sent on any cross-origin request. Even when        │
│      │   user clicks link from email to your site, cookie won't be sent     │
│      │   on that first request. Most secure but may affect user experience. │
│      │                                                                      │
│      ├── Lax:                                                               │
│      │   Cookie sent only on top-level navigation using safe HTTP methods   │
│      │   (GET). Not sent on cross-origin POST, iframe, AJAX, or image       │
│      │   requests. Good balance between security and usability.             │
│      │   Default in modern browsers.                                        │
│      │                                                                      │
│      ├── None:                                                              │
│      │   Cookie sent on all cross-origin requests (old browser behavior).   │
│      │   Must include Secure flag (HTTPS only). Use only when cross-site    │
│      │   cookie access is genuinely required (e.g., third-party widgets).   │
│      │                                                                      │
│      └── Example:                                                           │
...
│      │                                                                      │
│      ├── Origin Header:                                                     │
│      │   Sent by browser on POST/PUT/DELETE requests. Contains only         │
│      │   scheme and domain (https://example.com). Cannot be spoofed         │
│      │   by JavaScript. Reliable for identifying request source.            │
│      │                                                                      │
│      ├── Referer Header:                                                    │
│      │   Contains full URL of page that initiated request. Used as          │
│      │   fallback when Origin header is not present. Can be stripped        │
│      │   by privacy tools or referrer-policy settings.                      │
│      │                                                                      │
│      ├── Validation:                                                        │
│      │   Server maintains whitelist of allowed origins. Compares            │
│      │   incoming Origin/Referer against whitelist. Rejects request         │
│      │   if header is missing or doesn't match allowed domains.             │
│      │                                                                      │
│      └── Example:                                                           │
...
│      │                                                                      │
│      ├── Header:                                                            │
│      │   Require non-standard header like X-Requested-With: XMLHttpRequest  │
│      │   on all state-changing requests. This header must be explicitly     │
│      │   added by JavaScript code making the request.                       │
│      │                                                                      │
│      ├── Why It Works:                                                      │
│      │   HTML forms can only send standard Content-Types (text/plain,       │
│      │   multipart/form-data, application/x-www-form-urlencoded). Forms     │
│      │   cannot add custom headers. Only JavaScript can add custom headers. │
│      │                                                                      │
│      ├── CORS Effect:                                                       │
│      │   Cross-origin requests with custom headers are "preflighted".       │
│      │   Browser sends OPTIONS request first. If server doesn't allow       │
│      │   the custom header via CORS, actual request is blocked.             │
│      │                                                                      │
│      ├── Limitation:                                                        │
│      │   Only works for AJAX/fetch API requests. Cannot protect             │
│      │   traditional HTML form submissions. Best for API endpoints.         │
│      │                                                                      │
│      └── Example:                                                           │
│          // Client                                                          │
│          fetch('/api/transfer', {                                           │
...
│      │                                                                      │
│      ├── Set Cookie:                                                        │
│      │   Server generates random token and sets it as cookie. Cookie        │
│      │   must NOT have HttpOnly flag so JavaScript can read it.             │
│      │   Should have Secure and SameSite flags for additional security.     │
│      │                                                                      │
│      ├── Submit Token:                                                      │
│      │   Client-side JavaScript reads token from cookie and includes        │
│      │   it in request body or custom header. Both cookie and               │
│      │   body/header contain same token value.                              │
│      │                                                                      │
│      ├── Server Verification:                                               │
│      │   Server receives token in two places: automatically via cookie,     │
│      │   and manually via body/header. Server compares both values.         │
│      │   If they match, request is legitimate.                              │
│      │                                                                      │
│      ├── Why Secure:                                                        │
│      │   Attacker can trigger cross-origin request (cookie sent             │
│      │   automatically). But attacker cannot read victim's cookies          │
│      │   due to Same-Origin Policy. Therefore attacker cannot include       │
│      │   matching token in request body/header.                             │
│      │                                                                      │
│      └── Example:                                                           │
│          // Cookie                                                          │
│          Set-Cookie: csrf=abc123; SameSite=Strict; Secure                   │
│                                                                             │
│          // Client reads and submits                                        │
...
│      │                                                                      │
│      ├── Requirement:                                                       │
│      │   For high-risk operations, require user to re-enter current         │
│      │   password or complete 2FA verification before processing.           │
│      │   Acts as additional authentication layer.                           │
│      │                                                                      │
│      ├── Protected Actions:                                                 │
│      │   Password changes, email/phone updates, financial transactions,     │
│      │   account deletion, adding payment methods, modifying security       │
│      │   settings, downloading sensitive data exports.                      │
│      │                                                                      │
│      ├── Verification:                                                      │
│      │   Server validates provided credentials against stored credentials   │
│      │   before executing sensitive operation. If verification fails,       │
│      │   operation is rejected regardless of session validity.              │
│      │                                                                      │
│      ├── Why Effective:                                                     │
│      │   Even if attacker bypasses CSRF protection, they don't know         │
│      │   victim's current password. Forged request will fail at             │
│      │   re-authentication step. Defense in depth strategy.                 │
│      │                                                                      │
│      └── Example:                                                           │
